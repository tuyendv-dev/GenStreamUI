...
int performRtspHandshake(PSERVER_INFORMATION serverInfo) {
...

    {
        ...
        // GenStream patch: relay offset-forwarding -> override = base + (48000 - 47989).
        if (ServerPortBase != 0) {
            AudioPortNumber = ServerPortBase + (48000 - 47989);
            Limelog("Audio port: %u (derived from base %u)\n", AudioPortNumber, ServerPortBase);
        }
...

    {
        ...
        // GenStream patch: relay offset-forwarding -> override = base + (47998 - 47989).
        if (ServerPortBase != 0) {
            VideoPortNumber = ServerPortBase + (47998 - 47989);
            Limelog("Video port: %u (derived from base %u)\n", VideoPortNumber, ServerPortBase);
        }
    ...
    if (AppVersionQuad[0] >= 5) {
        ...
        // GenStream patch: relay offset-forwarding -> override = base + (47999 - 47989).
        if (ServerPortBase != 0) {
            ControlPortNumber = ServerPortBase + (47999 - 47989);
            Limelog("Control port: %u (derived from base %u)\n", ControlPortNumber, ServerPortBase);
        }
