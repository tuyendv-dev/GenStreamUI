...
int LiStartConnection(PSERVER_INFORMATION serverInfo, PSTREAM_CONFIGURATION streamConfig, PCONNECTION_LISTENER_CALLBACKS clCallbacks,
    ...
    // GenStream patch: base port của relay (0 = hành vi Moonlight gốc).
    ServerPortBase = serverInfo->serverPortBase;

    if (ServerPortBase != 0) {
        // Relay offset-forwarding: RTSP = base + (48010 - 47989).
        RtspPortNumber = ServerPortBase + (48010 - 47989);
        Limelog("RTSP port: %u (derived from base %u)\n", RtspPortNumber, ServerPortBase);
    }
