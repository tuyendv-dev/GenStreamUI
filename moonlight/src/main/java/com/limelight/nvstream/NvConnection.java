...
public class NvConnection {
    ...
    public NvConnection(Context appContext, ComputerDetails.AddressTuple host, int httpsPort, String uniqueId, StreamConfiguration config, LimelightCryptoProvider cryptoProvider, X509Certificate serverCert)
    {
        ...
        // GenStream: 0 = hành vi Moonlight gốc; set qua setServerPortBase() trước khi start().
    ...
    // GenStream: base port của relay; nếu != 0, moonlight-common-c suy RTSP/video/control/audio từ base.
    public void setServerPortBase(int serverPortBase) {
        this.context.serverPortBase = serverPortBase;
    }
    ...
    private boolean startApp() throws XmlPullParserException, IOException
    {
        ...
        if (!context.streamConfig.getApp().isInitialized()) {
            ...
            // GenStream fix: ghi appId resolved vào streamConfig — nếu không, launchNotRunningApp() dùng
            // context.streamConfig.getApp().getAppId() = 0 → launch?appid=0 → server đóng phiên (0x80030023).
            context.streamConfig.getApp().setAppId(app.getAppId());
            context.streamConfig.getApp().setHdrSupported(app.isHdrSupported());
        }
