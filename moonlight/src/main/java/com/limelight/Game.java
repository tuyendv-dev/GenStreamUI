...
public class Game extends Activity implements SurfaceHolder.Callback,
    ...
    // GenStream: base port của relay; nếu != 0, moonlight-common-c suy port stream từ base.
    public static final String EXTRA_SERVER_PORT_BASE = "ServerPortBase";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // GenStream: nếu chỉ có tên app (không kèm appId) → NvApp name-only (initialized=false) để
        // NvConnection tự getAppByName resolve trên host (vd "Desktop").
        if (appId == StreamConfiguration.INVALID_APP_ID && appName != null && !appName.isEmpty()) {
        ...
        // GenStream: cho phép thiếu appId nếu có tên app (resolve theo tên ở trên).
        if (appId == StreamConfiguration.INVALID_APP_ID && (appName == null || appName.isEmpty())) {
            finish();
            return;
        }
        ...
        // GenStream: nếu có base port của relay, để moonlight-common-c suy port stream từ nó.
        conn.setServerPortBase(getIntent().getIntExtra(EXTRA_SERVER_PORT_BASE, 0));
