...
public class ComputerManagerService extends Service {
    ...
    private ComputerDetails tryPollIp(ComputerDetails details, ComputerDetails.AddressTuple address) {
        try {
            ...
            int httpsPort = portMatchesActiveAddress ? details.httpsPort : 0;
            // GenStream relay offset-forwarding: HTTPS = base − 5. Ép dùng thẳng HTTPS để (1) bỏ bước dò
            // cổng qua HTTP base+0 (host GenStream không mở), (2) không tin httpsPort nội bộ (47984) mà
            // serverinfo báo về.
            if (details.derivePortsFromBase && address.port != 0) {
                httpsPort = address.port - 5;
            }

            NvHTTP http = new NvHTTP(address, httpsPort, idManager.getUniqueId(), details.serverCert,
            ...
            // GenStream: giữ cờ + ép httpsPort = base − 5 (cổng relay) thay httpsPort nội bộ từ serverinfo.
            if (details.derivePortsFromBase && address.port != 0) {
                newDetails.derivePortsFromBase = true;
                newDetails.httpsPort = address.port - 5;
            }
