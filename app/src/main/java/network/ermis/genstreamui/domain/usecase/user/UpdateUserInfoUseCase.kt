...
/**
 * UseCase cập nhật thông tin người dùng: map ResultWrapper + DTO -> UiState<User core>.
 */
class UpdateUserInfoUseCase @Inject constructor(
    private val userRepository: UserRepository
) {
    operator fun invoke(req: ReqUpdateUserInfo) = flow<UiState<User>> {
        emit(UiState.Loading)
        try {
            when (val response = userRepository.updateUserInformation(req)) {
